...
module.exports = {
    async addToTable_item_uris(uri, buy_limit) {
        try {
            await pool.query('INSERT INTO item_uris (uri, buy_limit) VALUES ($1, $2) ON CONFLICT (uri) DO NOTHING', [uri, buy_limit]);
        } catch (err) {
    ...
    async consume_item_uris(id) {
        try {
            ...
            return { uri: consumedID.rows[0].uri, buy_limit: consumedID.rows[0].buy_limit };
        } catch (err) {
